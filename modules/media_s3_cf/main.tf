...
resource "aws_s3_bucket" "bucket" {
  bucket        = "${var.project_name}-${var.env_name}-${var.media_bucket_name}-${random_integer.bucket_suffix.result}"
...
resource "aws_cloudfront_distribution" "distribution" {
  aliases     = ["media-${var.env_name}.${var.domain}"]
  web_acl_id  = var.web_acl_arn
  origin {
    domain_name = aws_s3_bucket.bucket.bucket_regional_domain_name
    origin_id   = "${var.project_name}-${var.env_name}-${var.media_bucket_name}-s3-origin"
    s3_origin_config {
  ...
  default_cache_behavior {
    ...
    target_origin_id = "${var.project_name}-${var.env_name}-${var.media_bucket_name}-s3-origin"
