  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      ...
      if kubectl get ingress ${self.triggers.project_name}-${self.triggers.env_name}-grafana -n monitoring >/dev/null 2>&1; then
          ...
          kubectl patch ingress ${self.triggers.project_name}-${self.triggers.env_name}-grafana \
            -n monitoring -p '{"metadata":{"finalizers":[]}}' --type=merge || true

          kubectl delete ingress ${self.triggers.project_name}-${self.triggers.env_name}-grafana \
            -n monitoring --force --grace-period=0 || true
      else
      ...
      if kubectl get ingress ${self.triggers.project_name}-${self.triggers.env_name}-argocd -n argocd >/dev/null 2>&1; then
          ...
          kubectl patch ingress ${self.triggers.project_name}-${self.triggers.env_name}-argocd \
            -n argocd -p '{"metadata":{"finalizers":[]}}' --type=merge || true

          kubectl delete ingress ${self.triggers.project_name}-${self.triggers.env_name}-argocd \
            -n argocd --force --grace-period=0 || true
      else
          echo "argocd ingress not found, skipping deletion"
      fi

      # Check if backend-api ingress exists before attempting deletion
      if kubectl get ingress ${self.triggers.project_name}-${self.triggers.env_name}-backend-api \
        -n ${self.triggers.env_name} >/dev/null 2>&1; then

          echo "Deleting backend-api ingress..."
          kubectl patch ingress ${self.triggers.project_name}-${self.triggers.env_name}-backend-api \
            -n ${self.triggers.env_name} -p '{"metadata":{"finalizers":[]}}' --type=merge || true

          kubectl delete ingress ${self.triggers.project_name}-${self.triggers.env_name}-backend-api \
            -n ${self.triggers.env_name} --force --grace-period=0 || true
      else
          echo "backend-api ingress not found, skipping deletion"
      fi
      ...
      for tg_arn in $(aws elbv2 describe-target-groups --region ${self.triggers.region} \
        ...
        [ ! -z "$tg_arn" ] && \
          aws elbv2 delete-target-group --target-group-arn "$tg_arn" --region ${self.triggers.region} || true
      done
        ...
        for sg_id in $sgs; do
          ...
          aws ec2 describe-security-groups --group-ids $sg_id --region ${self.triggers.region} \
            ...
            jq -c '.[]?' 2>/dev/null | while read rule; do
                [ ! -z "$rule" ] && \
                  echo "$rule" | aws ec2 revoke-security-group-ingress \
                    --group-id $sg_id --ip-permissions file:///dev/stdin \
          ...
          aws ec2 describe-security-groups --group-ids $sg_id --region ${self.triggers.region} \
            ...
            jq -c '.[]?' 2>/dev/null | while read rule; do
                [ ! -z "$rule" ] && \
                  echo "$rule" | aws ec2 revoke-security-group-egress \
                    --group-id $sg_id --ip-permissions file:///dev/stdin \
                    --region ${self.triggers.region} 2>/dev/null || true
            done
