...
#############################################
resource "random_id" "notification_suffix" {
  byte_length = 4
}
...
resource "aws_codestarnotifications_notification_rule" "pipeline_notifications" {
  ...
  name     = "${local.name_prefix}-${var.service_name}-notifications-${random_id.notification_suffix.hex}"
...
resource "aws_cloudwatch_log_group" "codebuild_logs" {
  name              = "/aws/codebuild/${local.name_prefix}-${var.service_name}-codebuild"
