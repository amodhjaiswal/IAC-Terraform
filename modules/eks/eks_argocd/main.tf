  ...
  values = [
    yamlencode({
      ...
      server = {
        ...
        # Run ArgoCD server in insecure mode since TLS is terminated at ALB
        extraArgs = [
          "--insecure"
        ]
      }
...
# Patch ArgoCD Ingress for HTTP Backend Protocol
resource "null_resource" "patch_argocd_ingress" {
  count = var.create_manifests ? 1 : 0
  
  provisioner "local-exec" {
    command = <<-EOT
      aws eks update-kubeconfig --name ${var.eks_cluster_name}
      kubectl patch ingress beeinrange-preprod-argocd -n argocd --patch '{"metadata":{"annotations":{"alb.ingress.kubernetes.io/backend-protocol":"HTTP"}}}' || true
    EOT
  }
